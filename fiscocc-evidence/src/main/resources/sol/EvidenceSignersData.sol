...
contract EvidenceSignersData{
		...
		function newEvidence(string evi, string info,string id,uint8 v, bytes32 r,bytes32 s, address sender)public returns(address)
        {
            Evidence evidence = new Evidence(evi, info, id, v, r, s, this, sender);
            newEvidenceEvent(evidence);
            return evidence;
        }
